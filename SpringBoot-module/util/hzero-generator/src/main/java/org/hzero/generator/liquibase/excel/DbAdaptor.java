...
public class DbAdaptor {
    ...
    static {
        ...
        SQL_TYPE_MAP.put("VARCHAR2", Types.VARCHAR);
        SQL_TYPE_MAP.put("MEDIUMTEXT", Types.VARCHAR);
        ...
        SQL_TYPE_MAP.put("NUMBER", Types.BIGINT);
        SQL_TYPE_MAP.put("INT", Types.BIGINT);
        SQL_TYPE_MAP.put("TINYINT", Types.BIGINT);
        SQL_TYPE_MAP.put("SMALLINT", Types.BIGINT);
        SQL_TYPE_MAP.put("INT8", Types.BIGINT);
        SQL_TYPE_MAP.put("INT4", Types.BIGINT);
        SQL_TYPE_MAP.put("INT2", Types.BIGINT);
        SQL_TYPE_MAP.put("TEXT", Types.VARCHAR);
        SQL_TYPE_MAP.put("LONGTEXT", Types.VARCHAR);
        SQL_TYPE_MAP.put("TIMESTAMP", Types.DATE);
        SQL_TYPE_MAP.put("DATETIME", Types.DATE);
    }
    ...
    public int processTableRow(TableData.TableRow tableRow) throws SQLException {
        ...
        // 值替换
        if (XmlUtils.ENABLE_REPLACE) {
            Set<Map.Entry<String, String>> data = XmlUtils.DATA_MAP.entrySet();
            for (TableCellValue tableCellValue : tableRow.getTableCellValues()) {
                for (Map.Entry<String, String> map : data) {
                    if (StringUtils.contains(tableCellValue.getValue(), map.getKey())) {
//                        tableCellValue.setValue(RegExUtils.replaceAll(tableCellValue.getValue(), map.getKey(), map.getValue()));
                        tableCellValue.setValue(StringUtils.replace(tableCellValue.getValue(), map.getKey(), map.getValue()));
                    }
                }
            }
        }
    ...
    protected Long doInsert(TableData.TableRow tableRow) throws SQLException {
            ...
            for (TableData.TableCellValue tableCellValue : tableRow.getTableCellValues()) {
                if (tableCellValue.getColumn().isGen()) {
                    ...
                    if (isGeneratedColumnInserted) {
                        ...
                        if (!StringUtils.contains(tableCellValue.getValue(), tableCellValue.getTableRow().getTable().getName())) {
                            // 主键为字符串
        ...
        if (genTableCellValue != null && !isGeneratedColumnInserted) {
    ...
    private void setParam(PreparedStatement ps,
        ...
        if (object == null) {
            try {
                ...
                logger.warn("*** SQL_TYPE_MAP没有定义匹配的类型 {}, 使用VARCHAR替换 ***", type);
                ps.setNull(nn, SQL_TYPE_MAP.get("VARCHAR"));
            }
    ...
    protected boolean checkTlExists(TableData.TableRow tableRow,
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            setParam(ps, genTableCellValue, 1);
            ps.setObject(2, lang);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next();
            } catch (SQLException e) {
