...
public class ScriptUtils {
    ...
    public void pullCreate(List<Service> serviceList) {
        serviceList.forEach(service -> {
            ...
            service.getExcelList().forEach(excel -> {
                ...
                FullExport liquibaseEngine = FullExport.createEngine(Constants.BASE_OUTPUT_PATH + service.getName() + "/" + excel.getSchema() + "/" + excel.getFileName() + ".xlsx", LiquibaseEngineMode.OVERRIDE);
    ...
    public void virtualMenuCreate(List<Service> serviceList) {
        serviceList.forEach(service -> {
            ...
            service.getExcelList().forEach(excel -> {
                LOGGER.info("开始处理：[{}]，使用数据库：[{}]", excel.getDescription(), excel.getSchema());
                VirtualMenuExport liquibaseEngine = VirtualMenuExport.createEngine(Constants.BASE_OUTPUT_PATH + service.getName() + "/" + excel.getSchema() + "/" + excel.getFileName() + ".xlsx", LiquibaseEngineMode.OVERRIDE);
                List<DataGroup> dataGroupList = create(service.getName(), excel);
                liquibaseEngine.setDataGroupList(dataGroupList);
                liquibaseEngine.generate();
            });
        ...
        serviceList.forEach(service -> {
            ...
            service.getExcelList().forEach(excel -> {
                ...
                for (String file : fileList) {
                    if (file.endsWith(SUFFIX_XLSX) && StringUtils.contains(file, service.getName() + "/" + excel.getSchema() + "/" + excel.getFileName())) {
                ...
                // 新增
                AppendExport appendExport = AppendExport.createEngine(Constants.BASE_OUTPUT_PATH + service.getName() + "/" + excel.getSchema() + "/" + excel.getFileName() + "-append.xlsx", LiquibaseEngineMode.OVERRIDE);
                appendExport.setTableDataList(tableDataList);
                appendExport.setDataGroupList(dataGroupList);
                appendExport.generate();
                // 删除
                /*DeleteExport deleteExport = DeleteExport.createEngine(Constants.BASE_OUTPUT_PATH + service.getName() + "/" + excel.getSchema() + "/" + excel.getFileName() + "-delete.xlsx", LiquibaseEngineMode.OVERRIDE);
