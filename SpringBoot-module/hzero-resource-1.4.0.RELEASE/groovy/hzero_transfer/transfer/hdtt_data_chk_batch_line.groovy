...
databaseChangeLog(logicalFilePath: 'script/db/hdtt_data_chk_batch_line.groovy') {
    changeSet(author: "jianbo.li@hand-china.com", id: "2019-08-05-hdtt_data_chk_batch_line") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'hdtt_data_chk_batch_line_s', startValue:"1")
        }
        createTable(tableName: "hdtt_data_chk_batch_line", remarks: "数据核对批次行") {
            column(name: "data_chk_batch_line_id", type: "bigint", autoIncrement: true ,   remarks: "表ID，主键")  {constraints(primaryKey: true)} 
            ...
            column(name: "tenant_id", type: "bigint",  remarks: "租户ID，按租户分发之用")   
            column(name: "process_status", type: "varchar(" + 3 * weight + ")",   defaultValue:"P",   remarks: "处理状态，参考：HDTT.PROCESS_STATUS")  {constraints(nullable:"false")}  
   ...
   createIndex(tableName: "hdtt_data_chk_batch_line", indexName: "hdtt_data_chk_batch_line_n1") {
            column(name: "data_chk_batch_id")
        }
